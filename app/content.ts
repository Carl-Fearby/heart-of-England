...
export function contentMedia(html:string){const seen=new Set<string>();const items:{src:string;alt:string}[]=[];for(const tag of html.match(/<img\b[^>]*>/gi)||[]){const src=tag.match(/\b(?:src|data-src)=["']([^"']+)/i)?.[1],normal=src?.replace(/^\/\//,"https://");if(!normal||!uploaded.test(normal))continue;const local=assetUrl(normal);if(seen.has(local))continue;seen.add(local);items.push({src:local,alt:decode(tag.match(/\balt=["']([^"']*)/i)?.[1]||"")});}return items;}
